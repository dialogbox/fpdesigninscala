...
class BloxorzSuite extends FunSuite {
  ...
  test("neighborsWithHistory for level 1") {
    new Level1 {
      ...
      assert(ns(0) == (Block(Pos(1,2),Pos(1,3)), List(Right,Left,Up)))
      assert(ns(1) == (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up)))
    }
  }
  ...
  test("newNeighborsOnly for level 1") {
    new Level1 {
      ...
      assert(ns.head == (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up)))
    }
  }
